...
// runeWidth returns 2 for CJK/wide characters, 1 otherwise
...
func (t *State) parse(c rune) {
	...
	// For wide characters, check if there's room for 2 cells
	if w == 2 && t.cur.X+1 >= t.cols {
		// No room for wide char, wrap to next line
	...
	if w == 2 && t.cur.X+1 < t.cols {
		...
		t.lines[t.cur.Y][t.cur.X+1] = t.cur.Attr
		t.lines[t.cur.Y][t.cur.X+1].Char = 0
