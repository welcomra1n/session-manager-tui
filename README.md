...
- **Session browser** — Discovers all Claude Code sessions from `~/.claude/projects/`, split into recent (last 7 days) and older with color coding (green / gray)
...
```bash
git clone https://github.com/borball/claude-session-tui.git
...
2. **Path decoding** — Resolves encoded directory names (e.g., `-Users-dev-projects-my-web-app`) back to real filesystem paths by walking the directory tree, correctly handling directory names with dashes
